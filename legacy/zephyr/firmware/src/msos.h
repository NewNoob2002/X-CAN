...
/* Zephyr development API used by the installed workspace. */
static struct net_buf *msos_request(const struct usbd_context *ctx,
                                    const struct usb_setup_packet *setup)
{
    ...
    size_t len = MIN(setup->wLength, sizeof(msos_set));
    struct net_buf *buf = usbd_ep_ctrl_data_in_alloc(ctx, len);
    if (buf != NULL) {
        net_buf_add_mem(buf, &msos_set, len);
    }
    return buf;
}
#else
